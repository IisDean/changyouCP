...
(function(){
	...
	cyou3.pageview = {
		...
		setPageLinkNumber: function(counts) {
			if (cyou3.pageview.checkPageObj()) {
				return;
			}
		...
		ellipsisClick: function() {
			...
			cyou3.pageview.updatePageLinkHtml(beginIndex);
		},
		...
		run: function(callback, errorCallback) {
			...
			cyou3.pageview.setNextAndPrevPage();
			cyou3.pageview.initPageLink();
			cyou3.pageview.refreshPageLinkCss();
			...
			pageChildNodes.click(function(){
				...
				if ($(this).attr("data") == _pageNumber) {
					...
					if (_pageAjaxURL == null) {
						cyou3.pageview.callbackExecute();
						return;
					} else {
		...
		callbackExecute: function() {
			cyou3.pageview.setNextAndPrevPage();
			cyou3.pageview.refreshPageLink();
			cyou3.pageview.ellipsisClick();
			cyou3.pageview.refreshPageLinkCss();
		},
		simplePage: function(pageLinks, totalPages, url, data, id, callback) {
			if (pageLinks != null && totalPages != null) {
				$(id).html("");
				cyou3.pageview.setPageArea(id);
				cyou3.pageview.setPageLinkNumber(pageLinks);
				cyou3.pageview.setPageNumberCounts(1, totalPages);
				cyou3.pageview.setPageLinkCss({"margin": "0px 6px", "color" : "", "font-weight": "normal"}, {"font-weight": "bold", "color": "#ff4800"});
				cyou3.pageview.setPageAjaxURL(url);
				cyou3.pageview.setPageJson(data);
				cyou3.pageview.run(callback, callback);
			}
		}
	};
})();