...
(function(){
	...
	cyou.pageview = {
		...
		ellipsisClick: function() {
			...
			cyou.pageview.updatePageLinkHtml(beginIndex);
		},
		...
		run: function(callback, errorCallback) {
			...
			cyou.pageview.setNextAndPrevPage();
			cyou.pageview.initPageLink();
			cyou.pageview.refreshPageLinkCss();
		...
		callbackExecute: function() {
			cyou.pageview.setNextAndPrevPage();
			cyou.pageview.refreshPageLink();
			cyou.pageview.ellipsisClick();
			cyou.pageview.refreshPageLinkCss();
		},
		simplePage: function(pageLinks, totalPages, url, data, id, callback) {
			if (pageLinks != null && totalPages != null) {
				$(id).html("");
				cyou.pageview.setPageArea(id);
				cyou.pageview.setPageLinkNumber(pageLinks);
				cyou.pageview.setPageNumberCounts(1, totalPages);
				cyou.pageview.setPageLinkCss({"margin": "0px 6px", "color" : "", "font-weight": "normal"}, {"font-weight": "bold", "color": "#ff4800"});
				cyou.pageview.setPageAjaxURL(url);
				cyou.pageview.setPageJson(data);
				cyou.pageview.run(callback, callback);
			}
		}
	};
})();